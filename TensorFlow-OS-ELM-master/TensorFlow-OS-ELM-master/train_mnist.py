...
def main():
    ...
    # load MNIST
    (x_train, t_train), (x_test, t_test1) = fashion_mnist.load_data()
    ...
    t_test = to_categorical(t_test1, num_classes=n_classes)
    ...
    y_pred = np.argmax(os_elm.predict(x_test), axis=-1)
    print(classification_report(t_test1, y_pred, target_names=class_names))
    print (end-start)
